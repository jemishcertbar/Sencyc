...
function updateSidebarToggleState() {
  ...
  if (sidebarToggle) {
    sidebarToggle.setAttribute("aria-label", isOpen ? "Expand navigation" : "Collapse navigation");
  ...
  if (mobileSidebarToggle) {
    ...
    mobileSidebarToggle.title = isOpen ? "Close navigation menu" : "Open navigation menu";
  }
}
// Close the sidebar and remember its collapsed state.
function closeSidebar() { sidebar.classList.add("collapsed"); sidebar.classList.remove("open"); sidebarBackdrop.classList.remove("visible"); localStorage.setItem("sencyc-sidebar-collapsed", "true"); updateSidebarToggleState(); }
// Open the sidebar and show the mobile backdrop when needed.
function openSidebar() { sidebar.classList.remove("collapsed"); sidebar.classList.add("open"); localStorage.setItem("sencyc-sidebar-collapsed", "false"); if (window.innerWidth <= 900) sidebarBackdrop.classList.add("visible"); updateSidebarToggleState(); }
// Apply the selected theme and save it for the next visit.
...
// Connect the mobile menu button after the sidebar exists.
function setupMobileToggle() {
  ...
  mobileSidebarToggle.setAttribute("aria-expanded", String(sidebar.classList.contains("open")));
}
...
// Build the detail page for one asset from the demo asset list.
...
// Build either the asset table or the findings table.
...
// Choose the page template, replace the view, then attach its event handlers.
